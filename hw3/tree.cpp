...
//  HW3
//
//  Created by Shreya Chatterjee on 5/10/19.
...
int countIncludes(const string a1[], int n1, const string a2[], int n2)
{
    ...
    if (a1[n1 - 1] == a2 [n2 - 1]) {
        //continue through current string to see if a2 exists in a1
        int current = countIncludes (a1, n1 - 1, a2, n2 - 1);
        //start recursion at next index of a1, to see if a2 exists again
        int nextIndex = countIncludes (a1, n1 - 1, a2, n2);
        //add values together
...
void order(string a[], int n)
{
    if (n > 1) {
        int index = n / 2;
        int firstNotLess = 0, firstGreater = n;
        separate(a, n, a[index], firstNotLess, firstGreater);   //divide array in half
        order(a, firstNotLess);     //sort left half
        order(a + firstNotLess + 1, n - firstGreater); //sort right half
    }
}
