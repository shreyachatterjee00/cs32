...
//  HW4
//
//  Created by Shreya Chatterjee on 5/26/19.
...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    for (int unsorted = 1; unsorted < s.size(); unsorted++) {
        ...
        //indexes 0 to unsorted - 1 are sorted
        //find the right location for the next element, shift, if necessary to make space
        
        Sensor nextSensor = s[unsorted];
        int loc = unsorted;
        while ((loc > 0) && !comp(s[loc - 1], nextSensor)) {
            //shift vector to the right
            s[loc] = s[loc - 1];
            loc--;
        } //end while
        
        //s[loc] is where nextSensor belongs
        s[loc] = nextSensor;
    } // end for
